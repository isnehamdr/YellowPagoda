...
export default function Navbar() {
  ...
  const [menuOpen,    setMenuOpen]    = useState(false);   // mobile offcanvas
  const [pagesOpen,   setPagesOpen]   = useState(false);   // mobile "Pages" accordion
  const [desktopDrop, setDesktopDrop] = useState(false);   // desktop hover dropdown
  ...
  // ── Desktop hover handlers (with small delay so cursor can reach dropdown) ─
  ...
  return (
    <>
      ...
      {/* ── OFFCANVAS ────────────────────────────────────────────────────────── */}
      <div
        className={`fixed top-0 right-0 h-full w-[300px] z-50 transform transition-transform duration-500 ease-in-out flex flex-col ${
      ...
      >
              ...
              <div key={item.label}>
                ...
                <div
                  className={`overflow-hidden transition-all duration-400 ease-in-out ${
                    pagesOpen ? "max-h-64 opacity-100 mt-1" : "max-h-0 opacity-0"
                  }`}
                >
                  <div className="flex flex-col gap-1 pl-3">
                    {PAGES_DROPDOWN.map((sub) => (
                      <Link key={sub.path} to={sub.path} onClick={() => setMenuOpen(false)}>
                        <button
        ...
        <div className="p-6 border-t border-white/10">
          <Link to="/book-now" onClick={() => setMenuOpen(false)}>
